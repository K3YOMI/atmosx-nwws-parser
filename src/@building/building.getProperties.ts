...
import { getExpiryDate } from "./building.getExpiryDate";
import { getIssuedDate } from "./building.getIssuedDate";
import { getOffice } from "./building.getOffice";
import { getTags } from "./building.getTags";
...
export const getProperties = (options: GetPropertiesOptions): TypeEventProperties => {
    const organization = options.message.match(RegularExpressions.wmo)?.[0] ?? null
    ...
    return { 
        ...
        geocode: {
            office: getOffice({ attributes: options.attributes, organization: organization, pVtec: options.pVtec }),
