...
This feature is intended for development, testing, and demonstration purposes by generating audio output from processed weather events without requiring access to broadcast equipment or live EAS systems. You can view a [sample generation here](../samples/SAMPLE_EAS_GENERATION.wav).
...
## Manual Trigger (`@setEasTone`)
You can manually trigger this generation by using the `setEasTone` function:
...
const output = await setEasTone({
	message: event.properties.description, 
	header: event.properties.metadata.header, 
	title: `severe_thunderstorm_warning_${Date.now()}`
})
/*
output = /ftp/@bucket/EAS/endocoded/
		 severe_thunderstorm_warning_1712345678.wav
*/
```
